...
class Evolution():
    ...
    def mutate(self, child):
        ...
        for param in child.nn.params:
            if p_m > np.random.uniform(0, 1):
                noise = np.random.normal(0, 0.8, child.nn.params[param].shape)
    ...
    def generate_new_population(self, num_players, prev_players=None):
        ...
        if prev_players is None:
            ...
            parents = self.q_tournament(prev_players, num_players, 4)
    ...
    def save_fitness(self, players):
        ...
        self.write_in_file(f"max_{self.mode}.csv", str(max_fitness))
        self.write_in_file(f"min_{self.mode}.csv", str(min_fitness))
        self.write_in_file(f"avg_{self.mode}.csv", str(avg_fitness))

    def write_in_file(self, filename, value):
        f = open('./plot_data/'+filename, "a")
        f.write(value)
        f.write('\n')
        f.close()